"""add_realtime_fields_to_seatings

Revision ID: 1a9227234fb8
Revises: 75a59bcc7858
Create Date: 2025-08-04 12:13:59.322519
...
revision: str = '1a9227234fb8'
down_revision: Union[str, Sequence[str], None] = '75a59bcc7858'
...
def upgrade() -> None:
    ...
    if 'seatings' in tables:
        columns_seatings = [col['name'] for col in inspector.get_columns('seatings')]
        if 'is_occupied' not in columns_seatings:
            op.add_column('seatings', sa.Column('is_occupied', sa.Boolean(), nullable=True))
        if 'occupied_at' not in columns_seatings:
            op.add_column('seatings', sa.Column('occupied_at', sa.DateTime(), nullable=True))
        if 'occupied_by' not in columns_seatings:
            op.add_column('seatings', sa.Column('occupied_by', sa.Integer(), nullable=True))
            # בדיקה אם ה-foreign key כבר קיים
            fks = [fk['name'] for fk in inspector.get_foreign_keys('seatings')]
            # נבדוק אם יש foreign key על occupied_by
            if not any('occupied_by' in str(fk) for fk in inspector.get_foreign_keys('seatings')):
                op.create_foreign_key(None, 'seatings', 'guests', ['occupied_by'], ['id'])
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_constraint(None, 'seatings', type_='foreignkey')
    op.drop_column('seatings', 'occupied_by')
    op.drop_column('seatings', 'occupied_at')
    op.drop_column('seatings', 'is_occupied')
