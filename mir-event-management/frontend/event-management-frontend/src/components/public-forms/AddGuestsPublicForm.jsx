  ...
  const validateField = (fieldName, value, options = {}) => {
    ...
    switch (fieldName) {
      case 'phone':
        ...
        break;
      case 'altPhone':
        if (value.trim() && !isPhoneValid(value, options.dialCode || altDialCode)) {
          error = 'מספר הטלפון אינו תקין';
        }
        break;
      case 'email':
          ...
          <div style={{ display: "flex", flexDirection: "column" }}>
            <div style={{ display: "grid", gridTemplateColumns: "120px 1fr", gap: 10 }}>
              ...
              <input 
                ...
                onBlur={() => validateField('altPhone', altPhone)}
                style={{ ...baseInputStyle, ...(validationErrors.altPhone ? { border: "2px solid #ef4444" } : {}) }} 
              />
            </div>
            {validationErrors.altPhone && <span style={{ color: '#ef4444', fontSize: 12, marginTop: 4 }}>{validationErrors.altPhone}</span>}
          </div>
          <div style={{ display: "flex", flexDirection: "column" }}>
            <div style={{ display: "grid", gridTemplateColumns: "120px 1fr", gap: 10 }}>
