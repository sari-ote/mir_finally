...
const TableVisual = ({ table, guests = [], isDragging, isViewer, onMouseDown, style = {}, label, categoryLabel = '', tableNumber, showGuestsList = false, onToggleGuestsList, hallType = 'm', onRemoveGuest }) => {
    ...
    return (
      <svg width={svgSize} height={svgSize} viewBox={`0 0 ${svgSize} ${svgSize}`} style={{ overflow: 'visible' }}>
        ...
        {Array.from({ length: totalChairCount }, (_, i) => {
          ...
          // מוזמן בכיסא הזה - רק אם באמת יש מוזמן באינדקס
          const guestAtSeat = Array.isArray(guests) ? guests[i] : undefined;
          const isOccupied = Boolean(guestAtSeat) || i < occupiedCount;
          ...
          if (isOverflowChair) {
            ...
            // כסא רגיל - ירוק רק אם יש מוזמן אמיתי
            fill = guestAtSeat ? '#10b981' : (i < occupiedCount ? '#10b981' : '#171717');
            stroke = guestAtSeat ? '#0e9f6e' : (i < occupiedCount ? '#0e9f6e' : '#333333');
            backFill = guestAtSeat ? '#0b7f5a' : (i < occupiedCount ? '#0b7f5a' : '#333333');
          }
          return (
            <g 
              key={i} 
              transform={`rotate(${rotation} ${x} ${y})`}
              style={{ cursor: guestAtSeat ? 'pointer' : 'default' }}
              onClick={(e) => {
                if (guestAtSeat && onRemoveGuest) {
                  e.stopPropagation();
                  onRemoveGuest(guestAtSeat);
                }
              }}
            >
              <rect x={x - 8} y={y - 6} width="16" height="12" fill={fill} rx="3" ry="3" stroke={stroke} strokeWidth="1" />
              <rect x={x - 8} y={y - 6} width="16" height="4" fill={backFill} rx="2" ry="2" />
              {guestAtSeat && (
                <title>{getDisplayName(guestAtSeat)} - לחץ להסרה</title>
              )}
            </g>
          );
        })}
  ...
  return (
    ...
    >
      ...
      <div onMouseDown={onMouseDown}>
        ...
        <div style={{ position: 'absolute', left: 0, top: 0, width: 120, height: 120, pointerEvents: 'none' }}>
          {Array.from({ length: totalChairCount }, (_, i) => (
