...
export default function NewDonorsPublicForm({ token, apiBase, formMeta, onSuccess, onError }) {
  ...
  const requiredErrors = () => {
    ...
    if (!details.phone.trim()) errors.push("יש להזין טלפון");
    if (!details.idNumber.trim()) errors.push("יש להזין תעודת זהות");
  ...
  const resetForm = () => {
    ...
    setMonths(24);
    setCurrency("ILS");
    setDetails({ firstName: "", lastName: "", idNumber: "", phone: "", email: "", gender: "", referral: "", city: "", street: "", apt: "" });
  ...
  const handleSubmit = async () => {
    ...
    const payload = {
      base: {
        ...
        building_number: details.apt.trim() || null,
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 16 }}>
        ...
        <input
          ...
          style={{
            ...baseInputStyle,
            ...(details.email.trim() && !isEmailValid(details.email) ? { border: "1px solid #ef4444" } : {}),
          }}
        />
        <select
