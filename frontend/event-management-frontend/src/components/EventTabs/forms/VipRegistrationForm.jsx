...
export default function VipRegistrationForm({ eventId }) {
	...
	async function handleSubmit() {
		...
		if (email.trim() && !isEmailValid(email)) {
			setShowErrors(true);
			setValidationMessage('האימייל שהוזן אינו תקין.');
			return;
		}
		try {
			setSaving(true);
		...
		<div style={{ background: '#f3f4f6', borderRadius: 16, padding: 20, position: 'relative' }}>
			...
			<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginTop: 12 }}>
				<div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: 10 }}>
					<select value={dialCode} onChange={e => setDialCode(e.target.value)} className="tropical-input" style={{ ...inputStyle, background: '#fff' }}>
			...
			{/* Row 4 */}
			<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginTop: 12 }}>
				<select value={blessingOption} onChange={e => setBlessingOption(e.target.value)} className="tropical-input" style={{ ...inputStyle, background: '#fff' }}>
				...
				<select value={donationAbility} onChange={e => setDonationAbility(e.target.value)} className="tropical-input" style={{ ...inputStyle, background: '#fff' }}>
			...
			<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginTop: 12 }}>
				<select value={extraGuestsMain === '' || extraGuestsMain === '0' || extraGuestsMain === 0 ? '' : extraGuestsMain} onChange={e => setExtraGuestsMain(e.target.value)} className="tropical-input" style={{ ...inputStyle, background: '#fff' }}>
				...
				<div style={{ marginTop: 12 }}>
					{extraGuests.map((g, idx) => (
						<div key={idx} style={{ marginTop: 8, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', gap: 12, background: '#f8fafc', padding: 12, borderRadius: 10 }}>
							...
							<select value={g.gender} onChange={e => updateExtra(idx, { gender: e.target.value })} className="tropical-input" style={{ ...inputStyle, background: '#fff' }}>
