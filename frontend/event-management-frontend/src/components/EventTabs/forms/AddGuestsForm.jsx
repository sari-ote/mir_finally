		...
		<div className="tropical-card" style={{ position: 'relative', padding: '24px' }}>
			...
			<div className="form-grid" style={{ marginTop: 12 }}>
				<input
					...
					style={{ ...inputStyle, ...(showErrors && !idNumber.trim() ? invalidStyle : {}) }}
				/>
			</div>
			...
			<div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginTop: 12 }}>
				...
				<div style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: 10 }}>
					...
					<input placeholder="מספר נייד" value={phone} onChange={e => setPhone(e.target.value)} className="tropical-input" />
				</div>
				<input placeholder="מייל" value={email} onChange={e => setEmail(e.target.value)} className="tropical-input" style={showErrors && email.trim() && !isEmailValid(email) ? invalidStyle : {}} />
			</div>
